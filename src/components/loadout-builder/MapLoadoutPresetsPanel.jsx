...
export function MapLoadoutPresetsPanel({
  ...
  async function handleLoad(saveId) {
    try {
      ...
      setMessage({ type: "success", text: "Loaded map preset." });
    } catch (error) {
      ...
      <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "start", flexWrap: "wrap" }}>
        <div style={{ display: "grid", gap: 4, maxWidth: 780 }}>
          ...
          <div style={{ fontSize: 13, color: colors.muted, lineHeight: 1.55 }}>
            Use the header save button to create or update the current single-map preset. These records include placements, map perks, placement bonuses, and spell loadout data for this map only.
          </div>
        </div>
        <button
          type="button"
